...
export class SourceJobWorker extends JobWorker<SourceAnalyzeJob> {
  ...
  protected async work() {
    ...
    const result = {
      ...
      artifactIds: uniq(payload.snapshotReport.scripts.map((s) => s.artifact?.id).filter((id): id is number => !!id)),
  ...
  private async prepareArtifacts() {
    ...
    for (const { artifactId, statsPath } of statsList) {
    ...
    files.push(
      ...(this.payload.snapshotReport.scripts ?? [])
        .filter((script) => script.artifact)
        .map((script) => ({
          fileName: script.url,
          bundleId: script.artifact!.id.toString(),
          diskPath: join(bundles[script.artifact!.id].dir, script.artifact!.filePath),
        })),
    )
